      ...
      <DialogContent className='max-h-[90vh] flex flex-col'>
          ...
          <form onSubmit={onSubmit}>
            <div className='grid grid-cols-8 gap-2'>
              <div className='col-span-4'>
                <InputCustom control={form.control} name='idCustomer' placeholder='Nhập id khách hàng' />
              </div>
        ...
        {customer && (
          <div className='p-1 border border-gray-200 rounded-lg shadow-md bg-white'>
            <h2 className='text-xl font-semibold text-gray-800 mb-2'>Thông tin khách hàng</h2>
            <p className='text-gray-700'>
              Tên khách hàng: <span className='font-medium'>{customer.customer_name}</span>
            </p>
            <p className='text-gray-700'>
              Mã khách hàng: <span className='font-medium'>{customer.customer_id}</span>
            </p>
            <p className='text-gray-700'>
              Bàn hiện đang ngồi: <span className='font-medium'>{customer.table_number}</span>
            </p>
          </div>
        )}
        ...
        <div className='flex-1 overflow-auto'>
          {productsChoosen?.map((productChoosen) => (
            <div key={productChoosen.product._id} className='mb-4 flex items-center justify-between'>
              <div className='flex items-center'>
                ...
                <div className='ml-2'>
                  ...
                  <p className='text-red-700 font-bold'>{formatCurrency(productChoosen.product.price)}đ</p>
                </div>
              </div>
              <div>
                <QuantityController value={productChoosen.buyCount} setValue={setValue(productChoosen.product._id)} />
              </div>
            </div>
          ))}
        </div>
          ...
          <>
            ...
            <div>
              <p className='mb-2 text-xl font-semibold'>
                Tổng tiền: <span className='font-bold text-2xl text-red-700'>{formatCurrency(totalPrice)}đ</span>
              </p>
