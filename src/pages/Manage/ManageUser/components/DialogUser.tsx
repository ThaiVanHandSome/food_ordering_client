...
export default function DialogUser({ user }: Props) {
  ...
  const updateUserMutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: UserRequest }) => updateUser(id, body)
  })
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{isUpdate ? 'Chỉnh sửa nhân viên' : 'Thêm nhân viên'}</DialogTitle>
        </DialogHeader>
        <div className='mb-3'>
          {previewImage && <img src={previewImage} alt={user?.avatar} className='size-36 rounded-lg mb-3' />}
            ...
            <form onSubmit={onSubmit}>
              <div className='grid grid-cols-4 gap-2'>
                ...
                <div className='col-span-3'>
                  <InputCustom control={form.control} name='name' placeholder='Tên' />
                </div>
              </div>
              <div className='grid grid-cols-4 gap-2'>
                <div className='col-span-1'>
                ...
                <div className='col-span-3'>
                  <InputCustom disabled={isUpdate} control={form.control} name='email' placeholder='Email' />
                </div>
              </div>
              <div className='grid grid-cols-4 gap-2'>
                <div className='col-span-1'>
              ...
              {isUpdate && (
                <div className='grid grid-cols-4 gap-2 mt-3'>
                  ...
                  <div className='col-span-3'>
                    <Switch id='airplane-mode' checked={isChangePassword} onCheckedChange={setIsChangePassword} />
                  </div>
                </div>
              )}
              {showPasswordInput && (
                <div className='grid grid-cols-4 gap-2 mt-3'>
                  ...
                  <div className='col-span-3'>
                    <InputCustom control={form.control} name='password' placeholder='Password' />
                  </div>
                </div>
              )}
