      ...
      <TableBody>
        {products.map((product) => (
          <TableRow key={product._id} className='font-semibold'>
            ...
            <TableCell>
              <img src={product.image} alt={product.name} className='size-16 rounded-lg object-cover mr-2' />
            </TableCell>
            <TableCell>{product.category.name}</TableCell>
            <TableCell>{product.name}</TableCell>
            <TableCell>{product.description}</TableCell>
            <TableCell className='italic text-destructive'>{formatCurrency(product.price)}đ</TableCell>
