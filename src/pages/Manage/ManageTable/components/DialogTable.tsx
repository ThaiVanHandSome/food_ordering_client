...
export default function DialogTable({ table }: Props) {
  ...
  const updateTableMutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: TableRequest }) => updateTable(id, body),
    onSuccess: (res) => {
      handleAfterSuccess(res.data.message)
    }
  })
  ...
  const url = `http://localhost:3000/table/${form.watch('tableNumber')}?token=${form.watch('token')}`
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      {isFormReset.current && (
        <DialogContent className='max-h-[90vh] overflow-y-auto'>
          <DialogHeader>
            <DialogTitle>{isUpdate ? 'Chỉnh sửa bàn ăn' : 'Thêm bàn ăn mới'}</DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={onSubmit}>
              ...
              <div className='grid grid-cols-4 gap-4'>
                ...
                <div className='col-span-3'>
                  <InputCustom control={form.control} name='capacity' placeholder='Nhập sức chứa' />
                </div>
              </div>
              <div className='grid grid-cols-4 gap-4 mb-4'>
                <div className='col-span-1'>Trạng thái</div>
                <div className='col-span-3'>
                  <SelectionCustom control={form.control} name='status' data={dataTableStatusSelection} />
                </div>
              </div>
              <div className='grid grid-cols-6 gap-4'>
                ...
                <div className='col-span-1'>
                  <Button size='icon' type='button' onClick={handleRefreshToken}>
              ...
              <Button disabled={addTableMutation.isLoading} className='mt-6 w-full'>
