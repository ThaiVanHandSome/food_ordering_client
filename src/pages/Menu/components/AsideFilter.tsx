...
export default function AsideFilter({ queryConfig }: Props) {
  ...
  return (
    <div className='px-6 py-4shadow-md rounded-md'>
      <div className='mb-6'>
        ...
        <ul className='space-y-2'>
          {categories?.data.data.map((category) => (
            <li
              key={category._id}
              className={clsx('hover:bg-primary hover:text-primary-foreground rounded-md', {
                'hover:bg-primary hover:text-primary-foreground': queryConfig.categoryId !== category._id,
                'bg-primary text-primary-foreground': queryConfig.categoryId === category._id
              })}
            >
              <Link
                to={{
      ...
      <div>
        ...
        <Form {...form}>
          <form onSubmit={onSubmit} className='space-y-4'>
            <InputCustom control={form.control} name='priceMin' placeholder='Từ' label='Giá tối thiểu' />
            <InputCustom control={form.control} name='priceMax' placeholder='Đến' label='Giá tối đa' />
